			...
			<div class="row">
				...
				<div class="col-sm-9 padding-right">
					<div class="features_items"><!--features_items-->
						...
						{
							$search=$_GET['search'];
							$result = mysqli_query($conn, "select count(masanpham) as total from sanpham where manhom='$manhom' && tensanpham like '%$search%'");
							...
							$sql="select * from sanpham where manhom='$manhom' && tensanpham like '%$search%' limit $start,$limit";
						}else if(isset($_GET['from']) && isset($_GET['to']))
						{
							$from=$_GET['from'];
							$to=$_GET['to'];
							$result = mysqli_query($conn, "select count(masanpham) as total from sanpham where manhom='$manhom' && gia>=$from && gia<=$to");
							$row = mysqli_fetch_assoc($result);
							$total_records = $row['total'];
							$config = array(
								'current_page'  => isset($_GET['page']) ? $_GET['page'] : 1,
							...
							$start=$paging->_config['start'];
							$limit=$paging->_config['limit'];
							$sql="select * from sanpham where manhom='$manhom' && gia>=$from && gia<=$to limit $start,$limit";
						}else{
							$result = mysqli_query($conn, "select count(masanpham) as total from sanpham where manhom='$manhom'");
							$row = mysqli_fetch_assoc($result);
							$total_records = $row['total'];
							$config = array(
								'current_page'  => isset($_GET['page']) ? $_GET['page'] : 1,
							...
							$start=$paging->_config['start'];
							$limit=$paging->_config['limit'];
							$sql="select * from sanpham where manhom='$manhom' limit $start,$limit";
						}
						...
						echo						'<p style="height:60px;line-height:20px;">'.$row["tensanpham"].'</p>
							...
							if(get_km($conn,$row['masanpham'])>0)
							{
