...
if( isset($_POST['thang'])){
	...
	if($ngay=="0")
	{
		$db=new DB_Connect();
		$conn=$db->connect();
		$sql="SELECT sanpham,count(soluong) as soluong,sum(gia*soluong) as thanhtien FROM `chitiethoadon` where mahoadon in(select mahoadon from hoadon where MONTH(ngaytao)='$_POST[thang]') group by sanpham";
		...
		$db=new DB_Connect();
		$conn=$db->connect();
		$sql="SELECT sanpham,count(soluong) as soluong,sum(gia*soluong) as thanhtien FROM `chitiethoadon` where mahoadon in(select mahoadon from hoadon where MONTH(ngaytao)='$_POST[thang]' and DAY(ngaytao)='$_POST[ngay]') group by sanpham";
		$result=$conn->query($sql);
		$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A1', 'Sản phẩm')
		->setCellValue('B1', 'Số lượng')
